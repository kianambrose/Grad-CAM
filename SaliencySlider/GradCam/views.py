...
def user_image(request):
    ...
    print('user tried submitting file')
...
def update_image(request):
    try:
        if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            print("Received intensity:", intensity)  # Debugging output
            print("Received image path:", image_path)  # Debugging output

            if not image_path:
                return JsonResponse({'error': 'Image path is empty'}, status=400)

            image_data_url = process_image(image_path, int(intensity))
            return JsonResponse({'image_data_url': image_data_url})
        else:
            return JsonResponse({'error': 'Invalid request'}, status=400)
    except Exception as e:
        print("Error processing image:", e)  # More detailed error logging
